...
@torch.no_grad()
def model_forward_wrapper(all_models, curr_obs, curr_delta, num_timesteps, latent_size, device, num_cond, num_goals=1, rel_t=None, progress=False, ddim=False):
...
def generate_rollout(args, output_dir, rollout_fps, idxs, all_models, obs_image, gt_image, delta, num_cond, device):
    ...
    for i in range(gt_image.shape[1]):
        ...
        if args.gt:
            ...
            x_pred_pixels = model_forward_wrapper(all_models, curr_obs, curr_delta, rollout_stride, args.latent_size, num_cond=num_cond, num_goals=1, device=device, ddim=True)
...
def generate_time(args, output_dir, idxs, all_models, obs_image, gt_output, delta, secs, num_cond, device):
    ...
    for sec, timestep in zip(secs, eval_timesteps):
        ...
        if args.gt:
            ...
            x_pred_pixels = model_forward_wrapper(all_models, obs_image, curr_delta, timestep, args.latent_size, num_cond=num_cond, num_goals=1, device=device, ddim=True)
        visualize_preds(output_dir, idxs, sec, x_pred_pixels)

def visualize_preds(output_dir, idxs, sec, x_pred_pixels):
    for batch_idx, sample_idx in enumerate(idxs.squeeze(1)):
...
def main(args):
    ...
    if not args.gt:
        ...
        if args.lora_dir != None:
            print("load lora.")
            model = PeftModel.from_pretrained(model, args.lora_dir, is_trainable=True)
        model.eval()
        model.to(device)
        model = torch.compile(model)
        diffusion = create_diffusion(args.diffusion_steps)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lora-dir", type=str, default=None, help="lora name")
    parser.add_argument("--diffusion_steps", type=str, default=str(250), help="diffusion time steps")
