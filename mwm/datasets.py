...
class SelfForcingTrainingDataset(BaseDataset):
    """
    Dataset for Self-Forcing (Algorithm 1) style training.

    Each sample provides:
      - context_images: the previous `context_size` GT frames ending at `curr_time`
      - target_images:  `rollout_len` GT future frames sampled within a window of size `len_traj_pred`
                        (not required to be adjacent)
      - actions_delta:  actions that move from the previous selected frame to the current selected frame
                        (computed in the same convention as EvalDataset: first convert to waypoints in
                        the coordinate frame of the start, then use get_delta_np)
      - rel_time:       normalized future offsets (offset/128.0), matching TrainingDataset convention
    """
    ...
    def __init__(
        ...
        # NOTE: `len_traj_pred` is interpreted as the *future sampling window size*.
        # You should set len_traj_pred >= rollout_len.
        super().__init__(
            data_folder,
            data_split_folder,
            dataset_name,
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        try:
            ...
            # Future sampling window: [curr_time + 1, curr_time + future_window]
            future_window = min(self.len_traj_pred, traj_len - curr_time - 1)
            if future_window <= 0 or future_window < self.rollout_len:
                raise ValueError(f"Trajectory too short at index={i}, curr_time={curr_time}, traj_len={traj_len}")

            # Sample `rollout_len` future offsets (no need to be adjacent). Prefer unique offsets.
            offsets = np.random.choice(
                np.arange(1, future_window + 1, dtype=np.int64),
                size=(self.rollout_len,),
                replace=False,
            )

            offsets = np.sort(offsets)
            rel_offsets = np.diff(np.concatenate(([0], offsets)))
            future_times = (curr_time + offsets).astype(np.int64)

            # Context frames: [curr_time-context_size+1, ..., curr_time]
            context_times = list(range(curr_time - self.context_size + 1, curr_time + 1))
            context_pairs = [(f_curr, t) for t in context_times]
            target_pairs = [(f_curr, int(t)) for t in future_times]

            context_images = torch.stack(
                [self.transform(Image.open(get_data_path(self.data_folder, f, t))) for f, t in context_pairs]
            )
            target_images = torch.stack(
                [self.transform(Image.open(get_data_path(self.data_folder, f, t))) for f, t in target_pairs]
            )
            ...
            for t in future_times:
                _, goal_pos = self._compute_actions(traj_data, cur, np.array([t]))
                goal_pos_list.append(goal_pos)
                cur = t
            goal_pos = np.concatenate(goal_pos_list, axis=0)
            goal_pos[:, :2] = normalize_data(goal_pos[:, :2], self.ACTION_STATS)

            rel_time = rel_offsets.astype(np.float32) / 128.0

            return (
                torch.as_tensor(context_images, dtype=torch.float32),
                torch.as_tensor(target_images, dtype=torch.float32),
                torch.as_tensor(goal_pos, dtype=torch.float32),
                torch.as_tensor(rel_time, dtype=torch.float32),
            )
        except Exception as e:
