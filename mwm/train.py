...
def main(args):
    ...
    if os.path.isfile(latest_path) or config.get('from_checkpoint', 0):
        ...
        if args.start_from_lora != None:
            ...
            model = PeftModel.from_pretrained(model, f"{checkpoint_dir}/{args.start_from_lora}", is_trainable=True)
        ...
        if "epoch" in latest_checkpoint:
            ...
            print("existing epoch:", start_epoch)
    ...
    diffusion = create_diffusion(timestep_respacing="")  # default: 1000 steps, linear noise schedule, spaced_diffusion
    eval_diffusion = create_diffusion(timestep_respacing=str(25))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y, rel_t in loader:
            ...
            if train_steps % args.eval_every == 0 and train_steps >= 0:
                ...
                #model.eval()
                sim_score = evaluate(ema, tokenizer, eval_diffusion, test_dataset, rank, config["batch_size"], config["num_workers"], latent_size, device, save_dir, args.global_seed, bfloat_enable, num_cond)
                # sim_score = evaluate(model, tokenizer, eval_diffusion, test_dataset, rank, config["batch_size"],
...
def evaluate(model, vae, diffusion, test_dataloaders, rank, batch_size, num_workers, latent_size, device, save_dir, seed, bfloat_enable, num_cond):
    ...
    for x, y, rel_t in loader:
        ...
        with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
            B, T = x.shape[:2]
            num_goals = T - num_cond
            samples = model_forward_wrapper((model, diffusion, vae), x, y, num_timesteps=None, latent_size=latent_size, device=device, num_cond=num_cond, num_goals=num_goals, rel_t=rel_t, ddim=False)
