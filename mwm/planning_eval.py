...
class WM_Planning_Evaluator:
    def __init__(self, args):
        ...
        if args.lora_dir != None:
            print("load lora.")
            model = PeftModel.from_pretrained(model, args.lora_dir, is_trainable=True).eval()
        self.model = torch.compile(model)
        self.diffusion = create_diffusion(str(5))
    ...
    def autoregressive_rollout(self, obs_image, deltas, rollout_stride):
        ...
        for i in range(deltas.shape[1]):
            ...
            x_pred_pixels = model_forward_wrapper(all_models, curr_obs, curr_delta, self.args.rollout_stride, self.latent_size, num_cond=self.num_cond, device=self.device, ddim=True)
