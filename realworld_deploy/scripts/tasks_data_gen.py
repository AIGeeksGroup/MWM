...
def generate_data_wrapper(i, track_num, nw, py_dir, py_path, use_gs, process_info, extra_args=""):
    ...
    try:
        ...
        if stdout:
            ...
            logger.debug(f"线程{i} 标准输出: {stdout.decode('gbk', errors='replace')}")
        if stderr:
            #logger.debug(f"线程{i} 错误输出: {stderr.decode('utf-8')}")
            logger.debug(f"线程{i} 错误输出: {stderr.decode('gbk', errors='replace')}")
...
if __name__ == "__main__":
    #py_dir = os.popen('which python3').read().strip() # linux, macos
    py_dir = sys.executable # windows
